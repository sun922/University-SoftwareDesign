...
| 컴퓨터공학과 홈페이지 개편 | TS-DRW-001 | 디자인 일관성 제공 기능     | 전문적 디자인 제공<br>- 학교 로고·폰트·버튼·여백 일관성 유지<br>- 참조 이미지 포함 | Y     | 프로젝트계획서 | 중   |
| 컴퓨터공학과 홈페이지 개편 | TS-DRW-001 | 해상도별 레이아웃 최적화     | 모바일·태블릿 최적화 레이아웃<br>- 화면 크기별 자동 조정                   | Y     | 프로젝트계획서 | 중   |
| 컴퓨터공학과 홈페이지 개편 | TS-DRW-002 | 모바일 전용 메뉴 제공      | 햄버거 메뉴·모바일 메뉴 지원<br>- 가독성 높은 폰트 자동 배치                | Y     | 프로젝트계획서 | 중   |
| 컴퓨터공학과 홈페이지 개편 | TS-DRW-003 | 터치 기반 요소 크기·간격 조절 | 모바일 터치 최적화<br>- 버튼 간격·크기 확보                          | Y     | 프로젝트계획서 | 중   |
| 컴퓨터공학과 홈페이지 개편 | TS-IMM-001 | 메뉴 구성 방식  | 상단 고정 메뉴·드롭다운 서브메뉴<br>- 주요 메뉴 상단 고정<br>- 서브메뉴 드롭다운      | Y     | 프로젝트계획서 | 중   |
| 컴퓨터공학과 홈페이지 개편 | TS-IAM-001 | 권한별 메뉴 표시 | 로그인·권한별 메뉴 동적 변경<br>- 비로그인 시 제한 메뉴 숨김<br>- 관리자 전용 메뉴 구성 a| Y     | 프로젝트계획서 | 중   |
| 컴퓨터공학과 홈페이지 개편 | TS-IFT-001 | 페이지 전환 흐름 | 목록→상세→댓글 흐름 제공<br>- 직관적 전환<br>- 버튼/링크 구분                | Y     | 프로젝트계획서 | 중   |
| 컴퓨터공학과 홈페이지 개편 | TS-IAC-001 | 접근성 보장 UI | 역할별 UI 차등 구성<br>- 불필요 정보 최소화<br>- 중요도 중심 UI             | Y     | 프로젝트계획서 | 중   |
| 컴퓨터공학과 홈페이지 개편 | TS-SFE-001 | 프론트엔드 프레임워크 | React/Vue.js SPA 적용<br>- CSR 빠른 전환<br>- 유지보수 용이 구조        | Y     | 프로젝트계획서 | 중   |
| 컴퓨터공학과 홈페이지 개편 | TS-SBE-001 | 백엔드 프레임워크   | Django/Node.js RESTful API<br>- 모듈화 구조<br>- ORM·비동기 처리 지원 | Y     | 프로젝트계획서 | 중   |
| 컴퓨터공학과 홈페이지 개편 | TS-SAP-001 | API 통신 방식   | RESTful 설계·HTTP 메서드 활용<br>- 명확한 URI 구조                    | Y     | 프로젝트계획서 | 중   |
| 컴퓨터공학과 홈페이지 개편 | TS-SJS-001 | 인증 및 세션 관리  | JWT 인증·세션 유지<br>- 토큰 로그인 인증<br>- 세션 만료 시간 설정              | Y     | 프로젝트계획서 | 중   |
