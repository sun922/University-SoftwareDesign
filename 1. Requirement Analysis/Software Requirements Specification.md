...
| 구분 |     고유번호        | 요구사항 명칭           | 설명                                                        |
| --- |--------------------- |------------- | --------------------------------------------------------- |
| NEX | NEX-001 |   구현 제외 조건      | 과제 목적상 실제 구현은 수행하지 않음                                     |
| NDL | NDL-001 |    제출 일정 제약      | 학기 내 보고서 제출 일정 엄수 필요                                      |
| NTM | NTM-001 |   팀 구성 제한       | 외부 인력 참여 없이 팀 내 작성 수행 *(외부 전문가 참여 금지 및 팀 내부 작성 필수)*       |
| BRC | BRC-001 |   브라우저 호환성 제약   | 크롬, 엣지, 사파리 최신 버전에서만 완전 지원<br>IE 미지원 및 기능·화면 동일 유지 필요     |
| OSS | OSS-001 |    오픈소스 사용 제한    | 검증된 오픈소스만 사용 가능<br>GPL 등 상업적 제약 라이선스 금지 및 관리자 승인 필요       |
| FRW | FRW-001 |   언어 및 프레임워크 조건 | 백엔드는 Python, 프론트엔드는 React.js 기반 구현<br>다른 스택 사용 시 사전 협의 필요 |
