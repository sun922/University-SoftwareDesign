...
| 고유번호           | 요구사항 명칭            | 권한                 |
| -------------- | ------------- | -- |
| **NSC-ENC-001** | 개인정보 저장 시 암호화 | 사용자 개개인 |
| **NSC-ENC-002** | 비밀번호 해시 저장 | 사용자 개개인 |

**보안 - SSL/HTTPS 통신 보안** (**NSC-SSL**)
| 고유번호           | 요구사항 명칭            | 권한                 |
| -------------- | ------------- | -- |
| **NSC-SSL-001** | SSL 인증서 적용 | 모두 |
...
| 고유번호           | 요구사항 명칭            | 권한                 |
| -------------- | ------------- | -- |
| **NSC-RIV-001** | 외부 실명 인증 API 연동 | 학생, 교수 |
...
| 고유번호           | 요구사항 명칭            | 권한                 |
| -------------- | ------------- | -- |
| **NSC-ACC-001** | 권한 기반의 페이지 접근 제어 | 모두 |
| **NSC-ACC-002** | 관리자 기능 접근 제한 | 최고 관리자 |

**보안 - 보안 로깅 및 추적** (**NSC-LOG**)
| 고유번호           | 요구사항 명칭            | 권한                 |
| -------------- | ------------- | -- |
| **NSC-LOG-001** | 로그인 기록 저장 | 최고 관리자 |
...
| 일련 | 고유번호                | 요구사항 명칭               | 권한 |
| -- | ------------------- | --------------------- | -- |
| 1  | **GEN-NMQ-001** | 장애 발생 시 10초 이내 복구     | 모두 |
| 2  | **GEN-NEX-001** | 시스템 가용성 보장 기능         | 모두 |
| 3  | **GEN-NUX-001** | 에러 발생 시 사용자 피드백 제공 기능 | 모두 |
| 4  | **GEN-NPR-001** | 시스템 반응 시간 제한 기능	모두 | 모두 |
...
| 일련 | 고유번호                | 요구사항 명칭       | 설명                                                        | 권한  |
| -- | ------------------- | ------------- | --------------------------------------------------------- | --- |
| 1  | **ENV-NEX-001** | 구현 제외 조건      | 과제 목적상 실제 구현은 수행하지 않음                                     | 모두  |
| 2  | **ENV-NDL-001** | 제출 일정 제약      | 학기 내 보고서 제출 일정 엄수 필요                                      | 모두  |
| 3  | **ENV-NTM-001** | 팀 구성 제한       | 외부 인력 참여 없이 팀 내 작성 수행 (외부 전문가 참여 금지 및 팀 내부 작성 필수)         | 모두  |
| 4  | **ENV-BRC-001** | 브라우저 호환성 제약   | 크롬, 엣지, 사파리 최신 버전에서만 완전 지원<br>IE 미지원 및 기능·화면 동일 유지 필요     | 모두  |
| 5  | **TEC-OSS-001** | 오픈소스 사용 제한    | 검증된 오픈소스만 사용 가능<br>GPL 등 상업적 제약 라이선스 금지 및 관리자 승인 필요       | 관리자 |
| 6  | **TEC-FRW-001** | 언어 및 프레임워크 조건 | 백엔드는 Python, 프론트엔드는 React.js 기반 구현<br>다른 스택 사용 시 사전 협의 필요 | 관리자 |
...
| 일련 | 고유번호           | 요구사항 명칭     | 권한  |
| -- | -------------- | ----------- | --- |
| 1  | **MTN-001** | 정기 유지보수 기능  | 모두  |
| 2  | **MTN-002** | 긴급 장애 대응 기능 | 모두  |
| 3  | **MTN-003** | CMS 제공 기능   | 관리자 |
