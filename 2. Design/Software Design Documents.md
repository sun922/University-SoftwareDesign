...
|------|--------------|--------------------------|------------------------------------|--------------------------------------------|-------------------------------------------|---------------------------------------------------------|
| FDO  | SW-FDO-001   | 학과 소개 모듈           | 학과 비전, 연혁, 구성 정보를 제공         | 없음                                       | 비전, 연혁, 구성 정보                         | 정적 JSON/DB 로딩, 렌더링 처리                             |
| FPR  | SW-FPR-001   | 교수진 소개 모듈         | 교수 개별 정보(이름, 연락처 등) 시각적 제공 | 없음                                       | 교수 목록(프로필, 전공, 연락처 등)             | 교수 DB 조회 (`SELECT * FROM professors`)                 |
| FCC  | SW-FCC-001   | 커리큘럼 안내 모듈       | 수업 목록과 강의계획서 안내                 | 없음                                       | 과목 목록, 강의계획서 링크                    | 커리큘럼 DB 조회 및 PDF 링크 제공                         |
| FNN  | SW-FNN-001   | 공지사항 열람 모듈       | 사용자에게 공지사항/뉴스 열람 기능 제공      | 없음                                       | 공지 제목, 작성일, 첨부파일 목록               | 공지 DB 조회, 최신순 정렬                                |
|      | SW-FNN-002   | 공지사항 등록/수정 모듈   | 관리자 공지 등록 및 수정 기능 제공           | 제목, 본문, 고정 여부, 첨부파일, 이미지            | 등록/수정 성공 여부                          | 공지사항 CRUD 처리, 파일 업로드, 관리자 권한 확인             |
| FRS  | SW-FRS-001   | 연구 성과 공유 모듈       | 논문, 특허, 프로젝트 등 성과 정보 열람       | 없음                                       | 논문 리스트, 프로젝트 요약                    | 연구성과 DB 조회 및 요약 가공                             |
| FAD  | SW-FAD-001   | 입학 안내 모듈           | 입학 일정 및 절차 안내                      | 없음                                       | 입학 요강, FAQ 링크                         | 정적 콘텐츠 로딩 또는 관리자 편집 콘텐츠 렌더링               |
| FCM  | SW-FCM-001   | 게시글 목록 모듈         | 게시글 리스트 조회 및 열람                   | 없음                                       | 게시글 제목, 작성자, 정렬 기준                 | 게시글 DB 조회, 고정/정렬 필터 적용                         |
|       | SW-FCM-002   | 게시글 작성 모듈         | 커뮤니티 글 등록 기능                       | 제목, 본문, 첨부파일                           | 등록 성공 여부                              | 게시글 등록 API 호출, 파일 업로드 처리                      |
|       | SW-FCM-003   | 게시글 상세 모듈         | 게시글 상세 확인 및 댓글 확인                | 게시글 ID                                 | 본문, 댓글 목록, 수정/삭제 버튼                | 게시글 상세 조회, 댓글 조회, 권한 확인                       |
| FAU  | SW-FAU-001   | 로그인 모듈             | 사용자 인증 처리                           | 이메일, 비밀번호                             | 인증 성공 여부, 세션 토큰                    | 사용자 DB 조회, 해시 검증, 세션 생성                         |
| FAR  | SW-FAR-001   | 권한 설정 모듈           | 사용자 역할별 접근 권한 설정                | 사용자 ID, 역할 설정 값                        | 저장 성공 여부                              | 권한 DB 업데이트, 관리자 권한 검증                          |
| FAA  | SW-FAA-001   | 실명 인증 모듈           | 사용자 실명을 기반으로 한 인증 절차 수행      | 이름, 주민등록번호 또는 학번, 사용자 유형         | 인증 결과, 오류 메시지                        | 외부 실명 인증 API 연동, 결과 반환, 오류 처리                 |
| FAM  | SW-FAM-001   | 콘텐츠 관리 모듈         | 공지/정적 콘텐츠 등록·수정 UI 제공           | 콘텐츠 내용, 수정 항목                          | 저장 성공 여부                              | CMS 저장소 연동, 콘텐츠 편집 이력 관리                       |
